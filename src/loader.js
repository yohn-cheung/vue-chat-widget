...
let win, instanceName, origin, loaderObject, tenant, defaultConfig;
...
export async function starting(window, config, scriptElement) {
	...
	win = window
	instanceName = scriptElement?.attributes.getNamedItem('id')?.value ?? DEFAULT_NAME;
	origin = scriptElement?.attributes.getNamedItem('id').ownerDocument.location.origin
	loaderObject = win[instanceName];
	tenant = loaderObject.q[0][1].tenant
